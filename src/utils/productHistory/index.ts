...
export const getProductHistoryUtil = async (idUser: string, limit?: number) => {
    try {
        return await new Promise((resolve, reject) => {
            dataBase.query(
              `SELECT products.idProducts, products.title, products.source FROM productHistory INNER JOIN products ON products.idProducts = productHistory.idProduct WHERE productHistory.idUser = '${idUser}' ORDER BY productHistory.updated_at DESC ${limit ? `LIMIT ${limit}` : ''};`,
              (err, data) => err ? reject(err) : resolve(data)
            );
          }) as Product[];
    } catch (error) {
...
export const updateProductHistoryUtil = async (idProduct: string, idUser: string, updated_at: string) => {
    try {
        return await new Promise((resolve, reject) => {
            dataBase.query(
...
export const DeleteProductHistoryUtil = async (idUser: string) => {
    try {
        return await new Promise((resolve, reject) => {
            dataBase.query(
              `DELETE FROM productHistory WHERE idUser = '${idUser}';`,
              (err, data) => err ? reject(err) : resolve(data)
            );
          });
    } catch (error) {
