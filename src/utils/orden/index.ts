...
export const createOrdenUtil = async (orden: Orden) => {
    try {
        return await new Promise((resolve, reject) => {
            dataBase.query(
              `INSERT INTO orden (idOrder, idCart, idUser, created_at, update_at, status, paymentMethod, shipping, discount, totalAmount, id_user_coupons, paymentId, qualified) VALUES ('${orden.idOrder}', '${orden.idCart}', '${orden.idUser}', '${orden.created_at}', '${orden.update_at}', '${orden.status}', ${orden.paymentMethod ? `'${orden.paymentMethod}'` : null}, '${orden.shipping}', '${orden.discount}', '${orden.totalAmount}', ${orden.id_user_coupons ? `'${orden.id_user_coupons}'` : null}, ${orden.paymentId ? `'${orden.paymentId}'` : null}, ${orden.qualified});`,
...
export const geteOrdenStatusUtil = async (idUser: string, status: string, page: number) => {
  try {
      return await new Promise((resolve, reject) => {
          dataBase.query(
            `SELECT idOrder, created_at, paymentMethod, paymentId, shipping, discount, totalAmount, idCart FROM orden WHERE idUser = '${idUser}' AND status = '${status}' ORDER BY created_at DESC LIMIT ${page}, 5;`,
            (err, data) => err ? reject(err) : resolve(data)
          );
        }) as Orden[];
  } catch (error) {
...
export const geteOrdensUtil = async (idPago?: string, page?: number) => {
  try {
      return await new Promise((resolve, reject) => {
          dataBase.query(
            `SELECT idOrder, idCart, idUser, created_at, update_at, paymentMethod, paymentId, shipping, discount, status, totalAmount, id_user_coupons FROM orden ${idPago ? `WHERE paymentId = '${idPago}'` : ''} ORDER BY created_at DESC LIMIT ${page}, 15;`,
            (err, data) => err ? reject(err) : resolve(data)
          );
        }) as Orden[];
  } catch (error) {
