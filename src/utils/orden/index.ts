...
export const createOrdenUtil = async (orden: Orden) => {
  try {
    return await new Promise((resolve, reject) => {
      dataBase.query(
        `INSERT INTO orden (idOrder, idCart, idUser, created_at, update_at, status, paymentMethod, shipping, discount, totalAmount, id_user_coupons, paymentId, qualified, numberOfOrder) VALUES ('${
          orden.idOrder
...
export const geteOrdensUtil = async (idPago?: string, page?: number) => {
  try {
    return (await new Promise((resolve, reject) => {
      dataBase.query(
        `SELECT idOrder, idCart, idUser, created_at, update_at, paymentMethod, paymentId, shipping, discount, status, totalAmount, id_user_coupons FROM orden ${
