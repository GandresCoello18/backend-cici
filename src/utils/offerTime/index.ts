...
export const getOfferTimeUtil = async () => {
  try {
    return (await new Promise((resolve, reject) => {
      dataBase.query(`SELECT * FROM offerTime ORDER BY created_at DESC;`, (err, data) =>
...
export const editOfferTimeUtil = async (
  finish_at: string,
  description: string,
  idOfferTime: string,
) => {
...
export const deleteOfferTimeUtil = async (idOfferTime: string) => {
  try {
    return await new Promise((resolve, reject) => {
      dataBase.query(`DELETE FROM offerTime WHERE idOfferTime = '${idOfferTime}';`, (err, data) =>
        err ? reject(err) : resolve(data),
      );
    });
  } catch (error) {
