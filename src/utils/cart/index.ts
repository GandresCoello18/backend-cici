...
export const getProductCartUtil = async (idCart: string) => {
  try {
      return await new Promise((resolve, reject) => {
          dataBase.query(
            `SELECT products.idProducts, products.source, products.title, products.price, cart_product.quantity FROM cart_product INNER JOIN products ON products.idProducts = cart_product.idProduct WHERE cart_product.idCart = '${idCart}';`,
            (err, data) => err ? reject(err) : resolve(data)
          );
        }) as Product[];
  } catch (error) {
