...
export const createCartProductUtil = async (cartProduct: CartProduct) => {
  try {
    return await new Promise((resolve, reject) => {
      dataBase.query(
        `INSERT INTO cart_product (id_cart_product, idProduct, quantity, idCart, colour) VALUES ('${
          cartProduct.id_cart_product
        }', '${cartProduct.idProduct}', ${cartProduct.quantity}, '${cartProduct.idCart}', ${
          cartProduct.colour ? `'${cartProduct.colour}'` : null
        });`,
        (err, data) => (err ? reject(err) : resolve(data)),
      );
    });
...
export const getCartProductUtil = async (idCart: string) => {
  try {
    return (await new Promise((resolve, reject) => {
      dataBase.query(`SELECT * FROM cart_product WHERE idCart = '${idCart}';`, (err, data) =>
...
export const UpdateProductCart = async (
  ...
  try {
    return await new Promise((resolve, reject) => {
      dataBase.query(
        `UPDATE cart_product SET quantity = ${quantity}, colour = ${
          colour ? `'${colour}'` : null
