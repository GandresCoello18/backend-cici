...
export const getCouponstUtil = async (option: { status?: string }) => {
  try {
    return (await new Promise((resolve, reject) => {
      dataBase.query(
        `SELECT * FROM coupons ${
          option.status ? `WHERE status = '${option.status}'` : ''
        } ORDER BY type DESC;`,
    ...
    return (await new Promise((resolve, reject) => {
      dataBase.query(
        `SELECT user_coupons.id_user_coupons, user_coupons.created_at, user_coupons.expiration_date, user_coupons.status, coupons.type, users.userName, users.avatar, invita.userName as user_name_invita, invita.avatar as user_avatar_invita FROM user_coupons LEFT JOIN users ON users.idUser = user_coupons.idUser LEFT JOIN coupons ON coupons.idCoupon = user_coupons.idCoupon LEFT JOIN users as invita ON user_coupons.idGuestUser = invita.idUser ${
          id_user_coupon ? `WHERE user_coupons.id_user_coupons = '${id_user_coupon}'` : ''
        } ORDER BY user_coupons.created_at DESC LIMIT ${page}, 15;`,
        (err, data) => (err ? reject(err) : resolve(data)),
      );
    })) as CouponsAssing[];
  } catch (error) {
...
export const getCouponsAssingUserUtil = async () => {
  try {
    return (await new Promise((resolve, reject) => {
      dataBase.query(`SELECT COUNT(*) / 15 as totalAssing FROM user_coupons;`, (err, data) =>
...
export const getCouponsUserFreetUtil = async (IdUser: string) => {
  try {
    return (await new Promise((resolve, reject) => {
      dataBase.query(`SELECT * FROM user_coupons WHERE idUser = '${IdUser}';`, (err, data) =>
        err ? reject(err) : resolve(data),
      );
    })) as MyCouponsUser[];
  } catch (error) {
...
export const createUserCouponsUtil = async (userCoupon: CouponsUser) => {
  try {
    return await new Promise((resolve, reject) => {
      dataBase.query(
        `INSERT INTO user_coupons (id_user_coupons, idUser, idCoupon, expiration_date, created_at, idGuestUser, status) VALUES ('${
          userCoupon.id_user_coupons
        }', ${userCoupon.idUser ? `'${userCoupon.idUser}'` : null}, ${
          userCoupon.idCoupon ? `'${userCoupon.idCoupon}'` : null
        }, '${userCoupon.expiration_date}', '${userCoupon.created_at}', ${
          userCoupon.idGuestUser ? `'${userCoupon.idGuestUser}'` : null
        }, '${userCoupon.status}')`,
        (err, data) => (err ? reject(err) : resolve(data)),
      );
    });
...
export const updateUserCouponsUtil = async (
  idCoupon: string,
...
export const DeleteCoupontUtil = async (idCoupon: string) => {
  try {
    return await new Promise((resolve, reject) => {
      dataBase.query(`DELETE FROM coupons WHERE idCoupon = '${idCoupon}';`, (err, data) =>
        err ? reject(err) : resolve(data),
      );
    });
  } catch (error) {
