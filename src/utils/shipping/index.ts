...
export const getCountShippingUtil = async () => {
  try {
    return (await new Promise((resolve, reject) => {
      dataBase.query(`SELECT COUNT(*) / 15 as totalShipping FROM shipping;`, (err, data) =>
...
export const getShippingUtil = async (idPago?: string, page?: number) => {
  try {
    return (await new Promise((resolve, reject) => {
      dataBase.query(
        `SELECT shipping.*, orden.paymentId, users.userName, users.avatar FROM shipping INNER JOIN orden ON orden.idOrder = shipping.idOrder INNER JOIN users ON users.idUser = orden.idUser ${
          idPago
            ? `WHERE orden.paymentId LIKE '%${idPago}%' OR shipping.guide LIKE '%${idPago}%'`
            : ''
        } ORDER BY shipping.created_at DESC LIMIT ${page}, 15;`,
        (err, data) => (err ? reject(err) : resolve(data)),
      );
    })) as Shipping[];
  } catch (error) {
...
export const createShippingUtil = async (Shipping: Shipping) => {
  try {
    return await new Promise((resolve, reject) => {
      dataBase.query(
        `INSERT INTO shipping (idShipping, idOrder, created_at, status, guide, method, update_at) VALUES ('${
          Shipping.idShipping
        }', '${Shipping.idOrder}', '${Shipping.created_at}', '${Shipping.status}', ${
          Shipping.guide ? `'${Shipping.guide}'` : null
        }, ${Shipping.method ? `'${Shipping.method}'` : null}, '${Shipping.update_at}');`,
        (err, data) => (err ? reject(err) : resolve(data)),
      );
    });
  } catch (error) {
...
export const updateStatusShippingUtil = async (
  status: string,
  update_at: string,
  idShipping: string,
) => {
